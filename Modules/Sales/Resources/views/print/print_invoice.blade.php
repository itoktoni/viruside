    ...
    <div id='page'>
        <div>
            ...
            <div id="box">
                ...
                <h2>
                    Invoice Date : {{ $master->sales_delivery_date_invoice ? $master->sales_delivery_date_invoice->format('d F Y') : '' }}
                </h2>
                <table>
                    <tr>
                        <td class="head">
        ...
        <div id="container">
            <table cellpadding="" 5 cellspacing="0" width="100%">
                ...
                <tr class="destination">
                    <td colspan='8'>
                        <strong>Invoiced To : {{ $master->order->customer->crm_customer_invoice_person ?? '' }}</strong>
                    </td>
                </tr>
                <tr class="contact">
                    <td colspan='8'>
                ...
                <tr>
                    <td colspan="8">
                        <p>
                            @php
                            $total = 0;
                            foreach($detail->where('sales_delivery_detail_out', '>', 0) as $tot){
                                $sum = $tot->sales_delivery_detail_price * $tot->sales_delivery_detail_out;
                                $total = $total + $sum;
                            }
                            $discount = $master->order->sales_order_discount_percent * $total / 100;
                            $after_discount = $total - $discount;
                            $tax = (($after_discount * $master->order->sales_order_tax_percent) / 100);
                ...
                @foreach($detail->where('sales_delivery_detail_out', '>', 0) as $item)
                <tr class="item">
                    <td class="no">
                        {{ $loop->iteration }}
                    </td>
